  ...
  for (let i = 0; i <= 20; i++) {
    ...
    for (let j = 0; j <= Math.floor(Math.random() * 4); j++) {
      const product = await database.Product.create({
        ...
        pictures: array_of(Math.floor(Math.random() * 3) + 1, () => m.random_element(list_of_photo)),
        hashtags: array_of(Math.floor(Math.random() * 3), () => m.random_element([
          'uniqlo', 'HandM', 'AIIZ', 'GAP', 'Crocs', 'anello', 'kanken',
        ])),
        colors: array_of(Math.floor(Math.random() * 3) + 1, () => {
          return { color: m.random_element(['red', 'blue', 'green']) };
        }),
        sizes: array_of(Math.floor(Math.random() * 3) + 1, () => {
          return { size: m.random_element(['S', 'M', 'L', 'XL', 'XXL']) };
        }),
        categories: array_of(Math.floor(Math.random() * 2) + 1, () => {
          return {
            category: m.random_element(
